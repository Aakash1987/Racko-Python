...
def print_intro():
	...
	You will be dealt 10 cards (Your Rack) out of a Racko deck, which is composed of 60 cards, each numbered 1 to 60.
...
def main():
	...
	while racko_flag is not True:
		...
		choice = raw_input("Do you want to take this card? y/n")
		...
		#If user does not want the discard pile card, show him the deck card
			...
			second_choice = raw_input("Do you want to take this card? y/n")
...
def find_and_replace(new_card,card_to_be_replaced,hand,discard):
	...
	while card_to_be_replaced not in hand:
		card_to_be_replaced = input("Please select a card in your hand to be replaced")
	for i in range(0,10):
...
def discard_or_deck(hand,discard,already_replaced):
	...
	card = discard[-1]
	if already_replaced[(60-card)/6] == 'n':
		return 'discard'
	else:
		return 'deck'
...
def discard_strategy(hand,discard,already_replaced):
	...
	card_to_be_replaced = hand[(60-card) / 6]
	already_replaced[(60-card)/6] = 'y'
	hand[(60-card) / 6] = card
	add_card_to_discard(card_to_be_replaced,discard)
